...
impl PfcDict {
    ...
    pub fn id(&self, s: &str) -> Option<u64> {
        ...
        while min <= max {
            ...
            let block_slice = &self.blocks.as_ref()[block_offset..];
            ...
            match s_bytes.cmp(head_slice) {
                ...
                Ordering::Equal => return Some((mid * BLOCK_SIZE) as u64),
            }
        }
...
impl<W: 'static + SyncableFile> PfcDictFileBuilder<W> {
    ...
    pub fn add_entry(&mut self, e: &PfcDictEntry) -> io::Result<u64> {
        let bytes = e.to_bytes();
        self.add_bytes(&bytes)
    }

    pub fn add(&mut self, s: &str) -> io::Result<u64> {
        let bytes = s.as_bytes();
        self.add_bytes(bytes)
    }

    pub fn add_bytes(&mut self, bytes: &[u8]) -> io::Result<u64> {
        if self.count % BLOCK_SIZE == 0 {
            if self.count != 0 {
                self.index.push(self.size as u64);
            }
            let len = write_nul_terminated_bytes(&mut self.pfc_blocks_file, bytes)?;
            self.size += len;
        } else {
            let common = find_common_prefix(&self.last.as_ref().unwrap(), bytes);
            let postfix = bytes[common..].to_vec();
            let common_len = vbyte::write_sync(&mut self.pfc_blocks_file, common as u64)?;
            let slice_len = write_nul_terminated_bytes(&mut self.pfc_blocks_file, &postfix)?;
    ...
    pub fn add_all_entries<I: 'static + Iterator<Item = PfcDictEntry> + Send>(
        &mut self,
        it: I,
    ) -> io::Result<Vec<u64>> {
        let mut result = Vec::new();
        for next in it {
            let r = self.add_entry(&next)?;
            result.push(r);
        }
        Ok(result)
    }

    pub fn add_all<'a, I: 'static + Iterator<Item = &'a str> + Send>(
        &mut self,
        it: I,
    ) -> io::Result<Vec<u64>> {
        let mut result = Vec::new();
    ...
    pub fn finalize(mut self) -> io::Result<()> {
        ...
        builder.push_vec(self.index)?;
        builder.finalize()?;

        write_padding(&mut self.pfc_blocks_file, self.size, 8)?;
        write_u64(&mut self.pfc_blocks_file, count)?;
        self.pfc_blocks_file.flush()?;
        self.pfc_blocks_file.sync_all()?;
...
pub fn dict_file_get_count<F: 'static + FileLoad>(file: &F) -> io::Result<u64> {
    ...
    let mapped = file.map()?;
    Ok(BigEndian::read_u64(&mapped[size - 8..]))
}
...
>(
    ...
    builder.add_all_entries(sorted_iterator)?;
    builder.finalize()
}
...
mod tests {
    ...
    fn can_create_pfc_dict_small() {
        ...
        builder.add_all(contents.into_iter()).unwrap();
        builder.finalize().unwrap();

        let p = PfcDict::parse(blocks.map().unwrap(), offsets.map().unwrap()).unwrap();
    ...
    fn can_create_pfc_dict_large() {
        ...
        builder.add_all(contents.clone().into_iter()).unwrap();
        builder.finalize().unwrap();

        let p = PfcDict::parse(blocks.map().unwrap(), offsets.map().unwrap()).unwrap();

        for (i, s) in contents.iter().enumerate() {
            assert_eq!(Some(s.to_string()), p.get(i));
        }
        assert_eq!(None, p.get(contents.len()));
    }

    #[test]
    fn can_search_pfc_dict() {
        let contents = vec![
            "aaaaa",
            "aabbb",
            "ccccc",
        ...
        for (i, s) in contents.iter().enumerate() {
            assert_eq!(Some(i as u64), p.id(s));
        }
        assert_eq!(None, p.id("zzz"));
    }
}
